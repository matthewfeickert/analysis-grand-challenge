...
def main(args, inputDataList):
    ...
    outputDB = do_the_muxing(args, database, listOfModels)

    # Handle the theory variation databses using the optimization from the nominal...

    if len(databaseListTheoryUp):
        outputDBTheoryUp = do_the_muxing(args, databaseTheoryUp, listOfModels, outputDB)
        outputDBTheoryDown = do_the_muxing(
    ...
    if len(databaseListUpperLimit):
        outputDBUpperLimit = do_the_muxing(
...
def do_the_muxing(args, database, listOfModels, nominalDatabase=0):
